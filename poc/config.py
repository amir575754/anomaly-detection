...
# Score normalization offset — maps sklearn decision_function range to [0, 1]
ISOLATION_FOREST_SCORE_OFFSET: float = 0.5
...
IMPLANT_BASELINE_MIN_DAYS: int = 30

# Seconds in one day — used to convert timedelta to days
SECONDS_PER_DAY: int = 86400

# Maximum number of feature vectors kept per (scope, config_type) sliding window in Redis
...
# Length of each detection batch window in minutes
DETECTION_WINDOW_MINUTES: int = 5

# Maximum telemetry rows scored per tick — prevents runaway ticks during backlog
...
# Groups and the number of implants in each; total defaults to 50
IMPLANT_GROUPS: dict[str, int] = {"ALPHA": 17, "BRAVO": 17, "CHARLIE": 16}
...
INGESTOR_LOG_INTERVAL_MESSAGES: int = 5000
